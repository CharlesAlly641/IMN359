...
regular = loadmat("regular.mat")['regular']
...
def erreur_approx(original, reconstructed):
    if np.linalg.norm(original) == 0:
        return np.inf  # Cas pathologique
...
def approximation_fourier_nonlineaire(image, M_values):
    ...
    for i, M in enumerate(M_values):
        ...
        axs[i].axis('off')
    plt.savefig('3_regular_fourier.jpg')
...
approximation_fourier_nonlineaire(regular, M_valeurs)
...
approximation_dct_nonlineaire(regular, M_valeurs)
...
def approximation_dct_locale_nonlineaire(image, M_values, block_size):
    ...
    plt.savefig('3_regular_dctloc.jpg')
...
approximation_dct_locale_nonlineaire(regular, M_valeurs, 32)
...
def approximation_ondelettes_nonlineaire(image, M_values, Jmax, Jmin):
    ...
    plt.savefig('3_regular_ondelettes.jpg')
    plt.show()

approximation_ondelettes_nonlineaire(regular, M_valeurs, 9, 1)